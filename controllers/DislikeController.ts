...
export default class DislikeController implements DislikeControllerI {
  ...
  /**
   * Check if a certain user has disliked a certain tuit
   * @param {Request} req Represents request from client, including the path
   * parameter tid and uid representing the disliked tuits id and the user id
  ...
  checkIfDisLike = async (req: Request, res: Response) => {
    ...
    await DislikeController.dislikeDao
      .findUserDislikesTuit(userId, tid)
      .then((isdislike) => res.json(isdislike));
  };
  ...
  userTogglesTuitDislikes = async (req: Request, res: Response) => {
    ...
    try {
      ...
      const howManyDislikedTuit = await dislikeDao.countHowManyDislikedTuit(
        tid
      );
